...
| `det/slogdet/solve_small_batch` | `np.linalg.det/slogdet/solve` on (..., 2, 2)/(..., 3, 3) float64 stacks, per-op batch floors 100-1000, inv_small_batch's det-vs-scale conditioning guard | det 2x2 up to 91.9x, det 3x3 up to 20.6x, slogdet up to 13.7x (signs exactly equal), Cramer solve up to 16.4x; numeric mode; singular/ill-conditioned stacks refused |
